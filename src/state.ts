...
import { MAX_LOG_ENTRIES, METADATA, ROOM_METADATA_LIMIT, TOKEN_LAYERS } from "./constants";
import {
  defaultSettings,
  defaultShop,
  sanitizeOrders,
...
type LocalData = Omit<SharedData, "catalog" | "version"> & { version: 2 };
...
function cacheLocalData(): void {
  ...
  catch (error) { console.error("[merchant] localStorage cache failed", error); }
}

function sharedPayload(): SharedData {
  return {
    version: 3,
...
function applySharedData(raw: unknown): boolean {
  if (!raw || typeof raw !== "object") return false;
  const value = raw as Partial<SharedData>;
  if (!value.settings || typeof value.settings !== "object") return false;
  const settings = sanitizeSettings(value.settings, state.lang);
  const catalogRaw = value.catalog as Partial<Catalog> | undefined;
  const catalog: Catalog = {
    items: Array.isArray(catalogRaw?.items) ? catalogRaw!.items as CatalogItem[] : [],
    services: Array.isArray(catalogRaw?.services) ? catalogRaw!.services as CatalogService[] : [],
  };
  const shops: Record<string, ShopData> = {};
  for (const [id, sourceValue] of Object.entries(value.shops ?? {})) {
  ...
  const wallets: Record<string, Wallet> = {};
  for (const [id, wallet] of Object.entries(value.wallets ?? {})) {
    wallets[id] = sanitizeWallet(wallet, id, `Player ${id.slice(0, 4)}`, "#8b8b9e", settings.currencies);
  }
  state.settings = settings;
  state.wallets = wallets;
  state.orders = sanitizeOrders(value.orders, settings.currencies);
  state.shops = shops;
...
async function saveSharedData(): Promise<boolean> {
  if (!state.roomKey) return false;
  const payload = sharedPayload();
  const serialized = JSON.stringify(payload);
  if (new Blob([serialized]).size > ROOM_METADATA_LIMIT) {
    toast("Os dados compartilhados desta sala ultrapassaram o limite de 16 KB do Owlbear. Exporte um backup e reduza a biblioteca.", "error");
    return false;
  }
  try {
    await OBR.room.setMetadata({ [METADATA.data]: payload });
    cacheLocalData();
    return true;
  } catch (error) {
    console.error("[merchant] shared data save failed", error);
    toast("Não foi possível salvar os dados compartilhados da sala.", "error");
    return false;
  }
}

async function loadSharedData(roomId: string): Promise<void> {
  try {
    const metadata = await OBR.room.getMetadata();
    if (applySharedData(metadata[METADATA.data])) return;
  } catch (error) {
    console.error("[merchant] shared data load failed", error);
  }

  const legacyCatalog = readCatalog();
  const local = loadLocalData(roomId, state.lang, legacyCatalog);
  state.settings = local.settings;
  state.wallets = local.wallets;
  state.orders = local.orders;
  state.shops = local.shops;
  state.catalog = legacyCatalog;
  cacheLocalData();
  if (state.role === "GM") await saveSharedData();
}
...
export function viewedWallet(): Wallet {
  const id = state.role === "GM" && state.walletViewId && state.wallets[state.walletViewId] ? state.walletViewId : state.playerId;
  return state.wallets[id] ?? myWallet();
}

export async function saveSettings(next: Settings): Promise<void> {
  if (state.role !== "GM") { toast("Somente o Mestre pode alterar as configurações da sala.", "error"); return; }
  state.settings = next;
...
export async function saveWallet(wallet: Wallet): Promise<void> {
  if (state.role !== "GM" && wallet.id !== state.playerId) {
    toast("Você só pode alterar sua própria carteira.", "error");
    return;
  }
  state.wallets = { ...state.wallets, [wallet.id]: { ...wallet, updatedAt: Date.now() } };
  await saveSharedData();
  requestRender();
}
export async function saveWallets(next: Record<string, Wallet>): Promise<void> {
  if (state.role !== "GM") { toast("Somente o Mestre pode alterar as carteiras de outros jogadores.", "error"); return; }
  state.wallets = next;
  await saveSharedData();
  requestRender();
}
export async function saveOrders(next: Order[]): Promise<void> { state.orders = next; await saveSharedData(); requestRender(); }
...
export async function updateShop(itemId: string, updater: (shop: ShopData) => ShopData | void): Promise<boolean> {
  ...
  state.shops[itemId] = compactShop(next); await saveSharedData();
...
export async function deleteShop(itemId: string): Promise<boolean> {
  ...
  delete state.shops[itemId]; await saveSharedData();
...
export function saveCatalog(catalog: Catalog): void {
  if (state.role !== "GM") { toast("Somente o Mestre pode alterar a biblioteca.", "error"); return; }
  state.catalog = catalog;
  writeCatalog(catalog);
  void saveSharedData();
  tokensSignature = "";
...
export function addLog(entry: Omit<LogEntry, "id" | "at">): void {
  state.settings = { ...state.settings, log: [{ id: uid("log"), at: Date.now(), ...entry }, ...state.settings.log].slice(0, MAX_LOG_ENTRIES) };
  void saveSharedData();
}
...
function parseRoute(): string | null {
  ...
  const createShopId = params.get("createShop");
  if (shopId) {
    const tab = params.get("tab") as ShopTab | null;
    const allowed: ShopTab[] = ["buy", "sell", "services", "manage"];
    state.route = { name: "shop", itemId: shopId, tab: tab && allowed.includes(tab) ? tab : "buy" };
  } else if (createShopId) {
    state.route = { name: "shop", itemId: createShopId, tab: "manage" };
  }
  return createShopId;
}
...
export function initApp(): void {
  OBR.onReady(async () => {
    ...
    await loadSharedData(roomId);
    const createShopId = parseRoute();
    if (createShopId && state.role === "GM") {
      try {
        const items = await OBR.scene.items.getItems();
        const token = items.find((item) => item.id === createShopId);
        await createShop(createShopId, token?.name ?? "");
      } catch (error) {
        console.error("[owlbear-merchant] nao foi possivel criar a loja:", error);
      }
    }
    await refreshTokens(); await ensureMyWallet(); state.ready = true; requestRender();
    OBR.room.onMetadataChange((metadata) => {
      if (metadata[METADATA.data]) applySharedData(metadata[METADATA.data]);
    });
